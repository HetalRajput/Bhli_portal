...
export default function BookingSuccessModal({
  ...
  return (
    <div className="booking-success-backdrop fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-[#03172d]/80 p-5 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="booking-success-title">
      <style>{`
      ...
      <section className="booking-success-card relative w-full max-w-[590px] overflow-hidden rounded-[2rem] border border-white/60 bg-white shadow-[0_35px_100px_rgba(0,16,38,.4)]">
        ...
        <div className="relative px-6 pb-7 pt-10 text-center sm:px-10 sm:pb-9 sm:pt-12">
          <div className="relative mx-auto grid size-24 place-items-center">
            ...
            <svg className="relative size-14" viewBox="0 0 56 56" fill="none" aria-hidden="true">
          ...
          <p className="mt-6 text-[11px] font-extrabold uppercase tracking-[.28em] text-emerald-600">Request submitted successfully</p>
          <h1 id="booking-success-title" className="mt-3 font-serif text-4xl leading-tight text-[#061f3b] sm:text-5xl">{heading}</h1>
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-500">{description}</p>

          <div className="mt-7 rounded-2xl border border-[#087fbe]/15 bg-gradient-to-br from-[#f3faff] to-[#edf8f6] p-4 text-left">
            <div className="flex items-start gap-3">
              <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-white text-[#087fbe] shadow-sm"><Building2 className="size-5" /></span>
            ...
            <div className="mt-4 grid gap-3 border-t border-[#087fbe]/10 pt-4 sm:grid-cols-2">
          ...
          <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-center">
            <Link href={backHref} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-slate-200 bg-white px-6 text-sm font-bold text-[#456078] transition hover:border-[#74bddb] hover:bg-[#f7fbfd]">{backLabel}</Link>
            <Link href="/profile?section=bookings" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#0875b7] to-[#13a5d8] px-7 text-sm font-bold text-white shadow-lg shadow-[#087fbe]/20 transition hover:-translate-y-0.5">View bookings <ArrowRight className="size-4" /></Link>
          </div>
          <p className="mt-5 flex items-center justify-center gap-2 text-[11px] text-slate-400"><Volume2 className="size-3.5" />Confirmation chime</p>
        </div>
      </section>
    </div>
  );
