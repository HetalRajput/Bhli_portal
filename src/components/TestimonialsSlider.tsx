...
export default function TestimonialsSlider({ testimonials }: { testimonials: Testimonial[] }) {
  return (
    <div className="relative" role="region" aria-roledescription="carousel" aria-label="Client testimonials">
      ...
      <div className="testimonial-marquee-mask overflow-hidden pb-5">
...
function TestimonialCard({ testimonial, position, total }: { testimonial: Testimonial; position: number; total: number }) {
  ...
  return (
    <article className="relative flex min-h-[440px] w-[min(84vw,390px)] shrink-0 flex-col overflow-hidden rounded-[2rem] border border-[#0a79bf]/10 bg-white p-7 shadow-[0_16px_50px_rgba(6,43,80,.08)] sm:w-[370px] sm:p-8 lg:w-[400px]" aria-label={`Testimonial ${position} of ${total}`}>
      <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[#087fbe] via-[#13a5d8] to-[#70d7ed]" />
      <Quote className="absolute right-6 top-7 size-16 text-[#087fbe]/[.07]" aria-hidden="true" />
      <span className="grid size-12 place-items-center rounded-2xl bg-[#e9f7fc] text-[#087fbe]"><Quote className="size-6" /></span>

      <div className="mt-6 flex gap-1" aria-label={`${rating} out of 5 stars`}>
        {Array.from({ length: 5 }, (_, starIndex) => (
          <Star key={starIndex} className={`size-4 ${starIndex < rating ? "fill-amber-400 text-amber-400" : "fill-transparent text-slate-200"}`} />
        ))}
      </div>
      ...
      <footer className="mt-8 border-t border-slate-100 pt-5">
        <p className="font-serif text-xl font-bold text-[#062b50]">{testimonial.name}</p>
