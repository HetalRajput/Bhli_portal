...
type Values = {
  ...
  nationality: string; traveller_type: string; number_of_travellers: string;
...
export default function TravelInsuranceRequestForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (new Date(values.dob) >= new Date()) return setError("Date of birth must be in the past.");
    ...
    Object.entries(requiredPayload).forEach(([key, value]) => formData.append(key, value));
    ...
    <div className="min-h-screen bg-[#edf5f9] pb-10 text-[#122b42]">
      ...
      <main className="relative z-10 mx-auto -mt-20 max-w-6xl px-5 lg:px-8">
        <div className="grid overflow-hidden rounded-[2rem] bg-white shadow-[0_28px_90px_rgba(6,31,59,.2)] lg:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <form id="travel-insurance-form" onSubmit={submit} className="p-5 sm:p-7 md:p-9" noValidate>
            ...
            <FormSection number="01" title="Traveller Details">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Full Name" required icon={<UserRound />}><input value={values.full_name} onChange={(e) => update("full_name", e.target.value)} autoComplete="name" /></Field>
                <Field label="Date of Birth" required icon={<CalendarDays />}><input type="date" max={new Date().toISOString().slice(0, 10)} value={values.dob} onChange={(e) => update("dob", e.target.value)} /></Field>
                <Field label="Mobile Number" required icon={<Phone />}><input type="tel" value={values.mobile} onChange={(e) => update("mobile", e.target.value)} autoComplete="tel" /></Field>
                <Field label="Email ID" required icon={<Mail />}><input type="email" value={values.email} onChange={(e) => update("email", e.target.value)} autoComplete="email" /></Field>
                <Field label="Passport Number" required icon={<FileCheck2 />}><input value={values.passport_number} onChange={(e) => update("passport_number", e.target.value)} /></Field>
                <SelectField label="Nationality" required icon={<Globe2 />} value={values.nationality} onChange={(value) => update("nationality", value)} loading={loadingCountries} placeholder="Select nationality">{countries.map((country) => <option key={country.id} value={country.country_code}>{country.name} ({country.country_code})</option>)}</SelectField>
              </div>
              <fieldset className="mt-4"><legend className="mb-2 text-xs font-bold text-[#456078]">Traveller Type <b className="text-red-500">*</b></legend><div className="flex flex-wrap gap-2">{[["adult", "Adult"], ["child", "Child"], ["senior_citizen", "Senior Citizen"]].map(([value, label]) => <Choice key={value} selected={values.traveller_type === value}><input className="sr-only" type="radio" name="traveller_type" checked={values.traveller_type === value} onChange={() => update("traveller_type", value)} />{label}</Choice>)}</div></fieldset>
              <div className="mt-4 max-w-xs"><Field label="Number of Travellers" required icon={<UserRound />}><input type="number" min="1" max="99" value={values.number_of_travellers} onChange={(e) => update("number_of_travellers", e.target.value)} /></Field></div>
            </FormSection>

            <FormSection number="02" title="Trip Details">
              <div className="grid gap-4 sm:grid-cols-2">
                <SelectField label="Insurance Type" required icon={<ShieldCheck />} value={values.insurance_type} onChange={(value) => update("insurance_type", value)} loading={loadingInsurance} placeholder="Select insurance type">{insuranceTypes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</SelectField>
                <SelectField label="Destination Country" required icon={<MapPin />} value={values.destination_country} onChange={(value) => update("destination_country", value)} loading={loadingCountries} placeholder="Select destination">{countries.map((country) => <option key={country.id} value={country.id}>{country.name}</option>)}</SelectField>
                <Field label="Trip Start Date" required icon={<CalendarDays />}><input type="date" min={new Date().toISOString().slice(0, 10)} value={values.trip_start_date} onChange={(e) => update("trip_start_date", e.target.value)} /></Field>
                <Field label="Trip End Date" required icon={<CalendarDays />}><input type="date" min={values.trip_start_date || new Date().toISOString().slice(0, 10)} value={values.trip_end_date} onChange={(e) => update("trip_end_date", e.target.value)} /></Field>
                <Field label="Purpose of Travel" required icon={<Plane />}><input value={values.purpose_of_travel} onChange={(e) => update("purpose_of_travel", e.target.value)} placeholder="Tourism, business, study…" /></Field>
                <SelectField label="Visa Type (if applicable)" icon={<FileCheck2 />} value={values.visa_type} onChange={(value) => update("visa_type", value)} loading={loadingVisas} placeholder="Select visa type">{visaTypes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</SelectField>
              </div>
            </FormSection>

            <FormSection number="03" title="Coverage Requirements">
              <div className="grid gap-2 sm:grid-cols-2">{coverageOptions.map((item) => <label key={item} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 text-xs font-bold transition ${coverage.includes(item) ? "border-sky-400 bg-sky-50 text-[#087fbe]" : "border-slate-200 text-slate-600 hover:border-sky-200"}`}><input type="checkbox" checked={coverage.includes(item)} onChange={() => toggleCoverage(item)} className="size-4 accent-[#087fbe]" />{item}</label>)}</div>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <Field label="Other Add-ons" icon={<HeartPulse />}><input value={values.other_add_ons} onChange={(e) => update("other_add_ons", e.target.value)} placeholder="Adventure sports cover" /></Field>
                <Field label="Coverage Amount Required" icon={<ShieldCheck />}><input value={values.coverage_amount_required} onChange={(e) => update("coverage_amount_required", e.target.value)} placeholder="e.g. USD 100,000" /></Field>
              </div>
            </FormSection>

            <FormSection number="04" title="Documents & Additional Information">
              <div className="grid gap-3 sm:grid-cols-3">
                <FileField label="Passport Copy" file={documents.passport_copy} onChange={(file) => chooseFile("passport_copy", file)} />
                <FileField label="Visa Copy" file={documents.visa_copy} onChange={(file) => chooseFile("visa_copy", file)} />
                <FileField label="Other Document" file={documents.other_document} onChange={(file) => chooseFile("other_document", file)} />
              </div>
              <label className="mt-4 block text-xs font-bold text-[#456078]">Remarks / Special Request<textarea value={values.remarks} onChange={(e) => update("remarks", e.target.value)} rows={4} placeholder="Share any health, itinerary or special coverage requirements." className="mt-2 w-full resize-y rounded-xl border border-slate-200 p-4 text-sm font-normal leading-6 outline-none focus:border-[#13a5d8] focus:ring-4 focus:ring-sky-100" /></label>
            </FormSection>

            <FormSection number="05" title="Declaration">
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-sky-100 bg-sky-50/70 p-4 text-xs leading-5 text-slate-600"><input type="checkbox" checked={declaration} onChange={(e) => setDeclaration(e.target.checked)} className="mt-0.5 size-4 shrink-0 accent-[#087fbe]" /><span>I confirm that the information provided is correct and authorize BHLI LLP to contact me regarding suitable travel insurance options. I understand that premiums, coverage, exclusions, deductibles and policy terms depend on the selected insurer and policy.</span></label>
            </FormSection>
...
function SelectField({ label, required, icon, value, onChange, loading, placeholder, children }: { label: string; required?: boolean; icon: ReactNode; value: string; onChange: (value: string) => void; loading: boolean; placeholder: string; children: ReactNode }) {
  return <label className="text-xs font-bold text-[#456078]">{label}{required && <b className="text-red-500"> *</b>}<span className="mt-2 flex h-12 items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 text-[#087fbe] transition focus-within:border-[#13a5d8] focus-within:ring-4 focus-within:ring-sky-100">{icon}<select value={value} onChange={(event) => onChange(event.target.value)} disabled={loading} className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-[#122b42] outline-none"><option value="">{loading ? "Loading..." : placeholder}</option>{children}</select></span></label>;
}
...
function FileField({ label, file, onChange }: { label: string; file: File | null; onChange: (file: File | null) => void }) {
  return <label className="flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-sky-200 bg-[#f8fcfe] p-4 text-center transition hover:border-sky-400"><FileUp className="size-5 text-[#087fbe]" /><b className="mt-2 text-xs text-[#173852]">{label}</b><span className="mt-1 max-w-full truncate text-[10px] text-slate-400">{file?.name || "PDF or image · max 10 MB"}</span><input type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={(event) => onChange(event.target.files?.[0] || null)} className="sr-only" /></label>;
}
