...
import { AlertCircle, Ban, CalendarDays, Clock3, Hash, LoaderCircle, MapPin, RefreshCw, ShieldCheck, Ticket, UsersRound, X } from "lucide-react";
    ...
    <>
      <div className="grid gap-3 sm:grid-cols-3">
        {[{ label: "Total requests", value: bookings.length, icon: Ticket, color: "text-[#0879b7] bg-[#edf8fd]" }, { label: "In progress", value: activeCount, icon: Clock3, color: "text-amber-700 bg-amber-50" }, { label: "Cancelled", value: cancelledCount, icon: Ban, color: "text-rose-700 bg-rose-50" }].map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><span className={`grid size-10 place-items-center rounded-xl ${color}`}><Icon className="size-5" /></span><span><b className="block text-lg text-[#07152d]">{value}</b><span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{label}</span></span></div>
        ))}
      </div>
      ...
      {!error && sortedBookings.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-[#0879b7]/25 bg-gradient-to-br from-[#f7fcff] to-white p-10 text-center">
        ...
        <div className="space-y-4">
          {sortedBookings.map((booking) => {
            ...
            return (
              <article key={booking.id} className="group overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition hover:border-[#0879b7]/20 hover:shadow-[0_16px_45px_rgba(6,70,110,.1)]">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="min-w-0 flex-1 p-5">
                    ...
                    <h3 className="mt-3 truncate text-base font-bold text-[#07152d]">{booking.service_item_title || primaryGuest || booking.service_name || "Booking request"}</h3>
                  ...
                  <div className="flex items-center justify-between gap-3 border-t border-dashed border-slate-200 bg-slate-50/60 p-5 md:w-48 md:flex-col md:justify-center md:border-l md:border-t-0">
      ...
      {selectedBooking && <BookingDetailModal booking={selectedBooking} showCancelForm={showCancelForm} cancelReason={cancelReason} cancelError={cancelError} cancelling={cancelling} setCancelReason={setCancelReason} setShowCancelForm={setShowCancelForm} cancelBooking={cancelBooking} close={() => setSelectedBooking(null)} />}
    </>
  );
}

function BookingDetailModal({ booking, showCancelForm, cancelReason, cancelError, cancelling, setCancelReason, setShowCancelForm, cancelBooking, close }: { booking: BookingRequest; showCancelForm: boolean; cancelReason: string; cancelError: string; cancelling: boolean; setCancelReason: (value: string) => void; setShowCancelForm: (value: boolean) => void; cancelBooking: () => void; close: () => void }) {
  const status = getStatus(booking.status);
  ...
  const closeModal = () => { setShowCancelForm(false); close(); };

  return (
    <div className="fixed inset-0 z-[110] flex items-end justify-center bg-[#021326]/80 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="booking-detail-title" className="flex max-h-[96dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-[2rem] border border-white/10 bg-[#f4f9fc] shadow-[0_35px_110px_rgba(0,16,38,.55)] sm:max-h-[90dvh] sm:rounded-[2rem]">
        <header className="relative flex shrink-0 items-start justify-between gap-5 overflow-hidden bg-[linear-gradient(125deg,#061a32_0%,#0a3155_58%,#0879b7_140%)] px-5 py-7 sm:px-8 sm:py-8">
          <div className="relative z-[1] min-w-0"><p className="text-[10px] font-extrabold uppercase tracking-[.22em] text-[#2ac2f1]">Booking request · BH{String(booking.id).padStart(6, "0")}</p><h2 id="booking-detail-title" className="mt-2 truncate font-serif text-2xl font-semibold text-white sm:text-3xl">{booking.service_item_title || booking.service_name || "Request details"}</h2><p className="mt-2 text-xs font-semibold text-white/55">Review your request details and latest reservation status.</p></div>
          <button type="button" onClick={closeModal} aria-label="Close booking details" className="relative z-[1] grid size-10 shrink-0 place-items-center rounded-full border border-white/15 bg-white/10 text-white transition hover:rotate-90 hover:bg-white/20"><X className="size-5" /></button>
        </header>

        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-7">
          <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-[#0879b7]/10 bg-white p-4 shadow-[0_10px_30px_rgba(14,72,105,.06)]"><span className="rounded-full border border-[#0879b7]/15 bg-[#e7f7fd] px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider text-[#0879b7]">{booking.service_name || formatLabel(booking.service_slug)}</span><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider ${status.chip}`}><span className={`size-2 rounded-full ${status.dot}`} />{status.label}</span><span className="w-full text-xs font-semibold text-slate-400 sm:ml-auto sm:w-auto">Submitted {formatDate(booking.created, true)}</span></div>
          ...
          {serviceFields.length > 0 && <section className="mt-6 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm"><h3 className="flex items-center gap-3 text-xs font-extrabold uppercase tracking-[.16em] text-[#07152d]"><span className="size-2 rounded-full bg-[#14a6d8]" />Service details</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{serviceFields.map(([key, value]) => <DetailValue key={key} label={formatLabel(key)} value={key.includes("date") ? formatDate(value) : value} />)}</div></section>}
          {booking.details && Object.keys(booking.details).length > 0 && <section className="mt-6 rounded-2xl border border-[#0879b7]/10 bg-white p-5 shadow-sm"><h3 className="flex items-center gap-3 text-xs font-extrabold uppercase tracking-[.16em] text-[#07152d]"><span className="size-2 rounded-full bg-[#0879b7]" />Preferences and budget</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{Object.entries(booking.details).map(([key, value]) => <DetailValue key={key} label={formatLabel(key)} value={value} accent />)}</div></section>}
          {booking.guests?.length > 0 && <section className="mt-6 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm"><h3 className="flex items-center gap-3 text-xs font-extrabold uppercase tracking-[.16em] text-[#07152d]"><span className="size-2 rounded-full bg-emerald-500" />Guests</h3><div className="mt-4 grid gap-2 sm:grid-cols-2">{booking.guests.map((guest, index) => <div key={guest.id || `${guest.name}-${index}`} className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-white font-extrabold text-[#0879b7] shadow-sm">{index + 1}</span><b className="min-w-0 flex-1 truncate text-[#07152d]">{guest.name}</b><span className="shrink-0 capitalize text-xs text-slate-500">{guest.age} yrs · {guest.gender}</span></div>)}</div></section>}
          {booking.message && <section className="mt-6 rounded-2xl border border-[#0879b7]/10 bg-gradient-to-br from-[#eaf8fe] to-white p-5 shadow-sm"><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#0879b7]">Your message</p><p className="mt-2 whitespace-pre-line text-sm leading-6 text-[#344a5c]">{booking.message}</p></section>}
          ...
          {canCancel && <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
          ...
          {!canCancel && <p className="mt-6 flex items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-xs font-semibold text-slate-500 shadow-sm"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#edf8fd]"><ShieldCheck className="size-4 text-[#0879b7]" /></span>This request is {status.label.toLowerCase()} and can no longer be cancelled online.</p>}
        </div>
      </section>
    </div>
  );
...
function DetailValue({ label, value, accent = false }: { label: string; value: unknown; accent?: boolean }) {
  return <div className={`rounded-xl border p-4 transition hover:-translate-y-0.5 hover:shadow-sm ${accent ? "border-[#0879b7]/10 bg-[#f2faff]" : "border-slate-100 bg-white"}`}><p className={`text-[9px] font-extrabold uppercase tracking-[.14em] ${accent ? "text-[#0879b7]/75" : "text-slate-400"}`}>{label}</p><p className="mt-1.5 break-words text-sm font-bold leading-5 text-[#07152d]">{formatValue(value)}</p></div>;
}
